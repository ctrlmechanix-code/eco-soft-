                        ...
                        {collectionPoints.map((point) => (
                            <motion.div
                                ...
                                className={`group p-5 rounded-2xl border cursor-pointer transition-all ${
                                    ...
                                    ? 'bg-emerald-50 border-emerald-500 shadow-md ring-1 ring-emerald-200' 
                                    : 'bg-white border-slate-200 hover:border-emerald-500 hover:shadow-md'
                                }`}
                            >
                                <div className="flex justify-between items-start">
                                    <h3 className={`font-bold text-lg transition-colors ${selectedPoint?.id === point.id ? 'text-emerald-700' : 'text-slate-900 group-hover:text-emerald-600'}`}>
                                        {point.name}
                                    </h3>
                                    <ArrowUpRight className={`w-5 h-5 transition-colors ${selectedPoint?.id === point.id ? 'text-emerald-500' : 'text-slate-300 group-hover:text-emerald-500'}`} />
                                </div>
                    ...
                    <AnimatePresence mode="wait">
                        {selectedPoint ? (
                            <motion.div
                                ...
                                exit={{ opacity: 0, x: 20 }}
                                className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 lg:p-10 flex flex-col h-full overflow-hidden"
                            >
                                <button 
                                    onClick={() => setSelectedPoint(null)}
                                    className="lg:hidden flex items-center gap-2 text-slate-400 hover:text-slate-900 mb-6 transition-colors"
                                >
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-start gap-6 mb-8">
                                    <div className="flex-grow">
                                        <h2 className="text-3xl font-bold text-slate-900 mb-2">{selectedPoint.name}</h2>
                                    ...
                                    <button 
                                        ...
                                        className="p-4 bg-emerald-50 rounded-2xl border border-emerald-100 text-emerald-600 hover:bg-emerald-100 transition-all flex items-center gap-2 font-bold shadow-sm"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm font-bold text-slate-900">{selectedPoint.email}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="h-full min-h-[150px] bg-slate-100 rounded-3xl overflow-hidden relative border border-slate-200">
                                         <img 
                                ...
                                <div className="mt-auto pt-8 border-t border-slate-100">
                                    ...
                                    <form onSubmit={handleSendMessage} className="relative group">
                                        <textarea 
                                            ...
                                            className="w-full px-5 py-4 pr-14 rounded-[1.5rem] bg-slate-50 border border-slate-200 focus:bg-white focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all text-slate-900 placeholder:text-slate-400 resize-none"
                                            rows={2}
                                        />
                                        <button 
                                            type="submit"
                                            disabled={!chatMessage.trim() || isSending}
                                            className="absolute right-3 bottom-3 p-3 bg-slate-900 text-white rounded-2xl hover:bg-emerald-600 disabled:bg-slate-200 disabled:text-slate-400 transition-all shadow-lg active:scale-95"
                                        >
                                            {isSending ? (
                                                <motion.div 
                                                    animate={{ rotate: 360 }}
                                    ...
                                    <AnimatePresence>
                                        {showSentSuccess && (
                                            <motion.p 
                                                ...
                                                animate={{ opacity: 1, y: 0 }}
                                                exit={{ opacity: 0 }}
                                                className="text-xs text-emerald-600 font-bold mt-2 flex items-center gap-1 justify-center"
                                            >
                            ...
                            <motion.div
                                ...
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                className="h-96 lg:h-full bg-slate-100 rounded-[2.5rem] overflow-hidden border border-slate-200 relative shadow-inner group"
                            >
                                <img 
                                ...
                                <div className="absolute top-6 right-6 bg-white/90 backdrop-blur-md px-4 py-2 rounded-xl text-xs font-bold shadow-sm text-slate-700 uppercase tracking-widest border border-slate-100">
                                    Interactive View
                                </div>
                            </motion.div>
                        )}
