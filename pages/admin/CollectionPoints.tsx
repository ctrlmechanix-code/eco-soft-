        ...
        <div>
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Manage drop-off locations and hours.</p>
                </div>
                <button 
                    onClick={handleAdd}
            ...
            <div className="grid gap-6">
                {points.map((point) => (
                    <div key={point.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        <div className="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 text-sm mb-2">{point.location} <span className="text-slate-300 mx-2">|</span> {point.address}</p>
                        ...
                        <div className="flex items-center gap-3 w-full md:w-auto">
                            <div className="mr-4 text-right hidden md:block">
                                ...
                                <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${point.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                                    {point.status || 'Active'}
                                </span>
                            </div>
                            <button 
                                onClick={() => handleEdit(point)}
                                className="flex-1 md:flex-none px-4 py-2 bg-slate-100 text-slate-700 rounded-lg font-medium hover:bg-slate-200 flex items-center justify-center gap-2"
                            >
