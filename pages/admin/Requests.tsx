        ...
        <div>
            <div className="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Manage user feedback and support tickets.</p>
                </div>
            </div>
            ...
            <div className="space-y-4">
                ...
                {requests.map((req) => (
                    <div key={req.id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                <span className={`px-2 py-1 rounded-full text-xs font-bold border ${
                                    req.status === 'Resolved' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
                                    req.status === 'In Progress' ? 'bg-blue-100 text-blue-700 border-blue-200' :
                        ...
                        <h3 className="text-lg font-bold text-slate-900 mb-2">{req.subject}</h3>
                        ...
                        {req.response && (
                            <div className="mb-4 pl-4 border-l-2 border-blue-200">
                                <p className="text-xs font-bold text-blue-600 mb-1">Your Response:</p>
                                <p className="text-sm text-slate-600">{req.response}</p>
                            </div>
                        )}
                        ...
                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-medium text-slate-700">{req.userName || 'User'}</span>
                            </div>
                            <div className="flex gap-2">
                                ...
                                {req.status !== 'Resolved' && (
                                    <button 
                                        ...
                                        className="text-sm font-medium text-emerald-600 hover:bg-emerald-50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                    >
